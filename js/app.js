    ...
    data.galeri.forEach((imgSrc, idx) => {
      ...
      const item = `
        <div class="overflow-hidden rounded-2xl cursor-pointer group shadow-sm hover:shadow-lg transition-all duration-500 relative lazy-svg-wrapper aos-init" data-aos="fade-up" data-aos-delay="${idx * 150}">
          <img src="${jpgSrc}" data-src-svg="${imgSrc}" alt="Prewedding Burhan & Fira ${idx + 1}" class="w-full h-auto object-cover transition-transform duration-700 ease-out group-hover:scale-110 lazy-svg grayscale group-hover:grayscale-0 transition-all" loading="lazy" />
